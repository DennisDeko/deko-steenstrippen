...
def draw_strip_diagram(A, B, C, D, x_cuts=None, y_cuts=None, zool_cuts=None):
    ...
    if zool_cuts: patches.append(mpatches.Patch(facecolor=ZCOL,edgecolor=ZLINE,label="Zool-snijvlak (hoogte)"))
    ax.legend(handles=patches,loc="lower right",fontsize=8.5,framealpha=0.95,edgecolor="#d1d5db",bbox_to_anchor=(0.98,0.02))

    ax.set_title("Steenstrippen – Afgekorte strip",fontsize=11,fontweight="bold",color="#1a1a2e",loc="left",pad=10)
    ax.autoscale_view()
    xl=ax.get_xlim(); yl=ax.get_ylim()
    ax.set_xlim(xl[0]-1.8,xl[1]+3.2); ax.set_ylim(yl[0]-1.4,yl[1]+1.8)
